...
if ($result) {
	...
	while ($row = sqlsrv_fetch_array($result)) {
		...
		if ($bikin == '1') {
			if (strlen(rtrim($csldb)) == 7 || strlen(rtrim($cslcr)) == 7) {
				// $tulis = substr($ctggl, 0, 4) . substr($ctggl, 5, 2) . substr($ctggl, 8, 2);
				$tulis = substr($ctggl->format('Y-m-d'), 0, 4) . substr($ctggl->format('Y-m-d'), 5, 2) . substr($ctggl->format('Y-m-d'), 8, 2);

				$tulis .= substr($ctggl->format('Y-m-d'), 0, 4) . substr($ctggl->format('Y-m-d'), 5, 2) . substr($ctggl->format('Y-m-d'), 8, 2);
				$tulis .= substr($kdwil, 0, 2) . $kdcab . substr($csldb, 0, 7) . "IDR D" . $singk . substr($ckode, 0, 3) . substr($cnota, 0, 15) . " ";
				...
				$tulis = substr($ctggl->format('Y-m-d'), 0, 4) . substr($ctggl->format('Y-m-d'), 5, 2) . substr($ctggl->format('Y-m-d'), 8, 2);
				$tulis .= substr($ctggl->format('Y-m-d'), 0, 4) . substr($ctggl->format('Y-m-d'), 5, 2) . substr($ctggl->format('Y-m-d'), 8, 2);
				$tulis .= substr($kdwil, 0, 2) . $kdcab . substr($csldb, 0, 5) . "IDR   D" . $singk . substr($ckode, 0, 3) . substr($cnota, 0, 15) . " ";
				$tulis .= pad_zero_18($cjmlh);
				$tulis .= pad_zero_19($cjmlh);
				$tulis .= " " . str_pad($cdesk, 30) . "\r\n";
