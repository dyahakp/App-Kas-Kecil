		...
		<div class="row justify-content-center">
			<div class="col-md-12">
				<div class="card mt-5">
					...
					$sql = "SELECT TOP 1 BLCLBON FROM BLMASTER WHERE BLTOPC = '$trstopc' AND BLDATE < '$txttgl1' ORDER BY BLDATE DESC";
					$result = sqlsrv_query($conn2, $sql);
		...
		<div id="result">
			...
			if (isset($_GET['cari'])) {
				...
				$sql = "SELECT TRSNOTA, TRSCODE, TRSVALUTA, TRSDESC, TRSDBCR, TRSNILAI, TRSREFF, TRSTORF, TRSUSRINP, TRSUSRAUT
      					FROM TRMASTER
     					WHERE TRSVALUTA >= '$txttgl1' AND TRSVALUTA <= '$txttgl2' AND TRSCODE = '$kasbon' AND TRSTOPC = '$trstopc' AND ISNULL(TRSUSRAUT, SPACE(10)) != SPACE(10) AND (TRSDESC LIKE '%$keyword%' OR TRSREFF LIKE '%$keyword%' OR TRSCODE LIKE '%$keyword%')
      					ORDER BY TRSVALUTA, TRSREFF";
			} elseif (isset($_GET['submit-date'])) {
				$sql = "SELECT TRSNOTA, TRSCODE, TRSVALUTA, TRSDESC, TRSDBCR, TRSNILAI, TRSREFF, TRSTORF, TRSUSRINP, TRSUSRAUT
					FROM TRMASTER
					WHERE TRSVALUTA >= '$txttgl1' AND TRSVALUTA <= '$txttgl2' AND TRSCODE = '$kasbon' AND TRSTOPC = '$trstopc' AND ISNULL(TRSUSRAUT, SPACE(10)) != SPACE(10)
					ORDER BY TRSVALUTA, TRSREFF";
			} elseif (isset($_GET['filter-kredit'])) {
				$sql = "SELECT TRSNOTA, TRSCODE, TRSVALUTA, TRSDESC, TRSDBCR, TRSNILAI, TRSREFF, TRSTORF, TRSUSRINP, TRSUSRAUT
					FROM TRMASTER
					WHERE TRSVALUTA >= '$txttgl1' AND TRSVALUTA <= '$txttgl2' AND TRSCODE = '$kasbon' AND TRSTOPC = '$trstopc' AND ISNULL(TRSUSRAUT, SPACE(10)) != SPACE(10) AND (TRSDBCR LIKE '%k%')
					ORDER BY TRSVALUTA, TRSREFF";
			} elseif (isset($_GET['filter-debit'])) {
				$sql = "SELECT TRSNOTA, TRSCODE, TRSVALUTA, TRSDESC, TRSDBCR, TRSNILAI, TRSREFF, TRSTORF, TRSUSRINP, TRSUSRAUT
					FROM TRMASTER
					WHERE TRSVALUTA >= '$txttgl1' AND TRSVALUTA <= '$txttgl2' AND TRSCODE = '$kasbon' AND TRSTOPC = '$trstopc' AND ISNULL(TRSUSRAUT, SPACE(10)) != SPACE(10) AND (TRSDBCR LIKE '%d%')
					ORDER BY TRSVALUTA, TRSREFF";
			}
